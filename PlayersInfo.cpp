...
json GetServerInfo()
{
	...
	for (int i = 0; i < 64; i++)
	{
		...
		if(g_pLRCore) jPlayer["rank"] = g_pLRCore->GetClientInfo(i, ST_RANK);
...
CON_COMMAND_F(mm_getinfo, "", FCVAR_GAMEDLL)
{
	...
	char szBuffer[1024];
	g_SMAPI->Format(szBuffer, sizeof(szBuffer), "%s\n", jdata.dump().c_str());
	META_CONPRINT(szBuffer);
}
...
void PlayersInfo::AllPluginsLoaded()
{
	...
	if (ret == META_IFACE_FAILED) g_pLRCore = nullptr;
